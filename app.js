...
const BACKEND_URL = 'https://your-app.up.railway.app'; // ← paste your Railway URL here
...
function setMode(mode) {
  ...
  currentMode = mode;
  document.getElementById('pill-photo').classList.toggle('active', mode === 'photo');
  document.getElementById('pill-video').classList.toggle('active', mode === 'video');
...
async function saveToDrive() {
  ...
  const mime = capturedBlob.type || (capturedType === 'photo' ? 'image/jpeg' : 'video/webm');
  const ext  = mime.includes('mp4')  ? 'mp4'
             : mime.includes('webm') ? 'webm'
             : mime.includes('jpeg') || mime.includes('jpg') ? 'jpg'
             : mime.includes('png')  ? 'png'
             : capturedType === 'photo' ? 'jpg' : 'webm';

  const filename = EVENT_NAME.replace(/[^a-zA-Z0-9]/g, '_') + '_' + ts + '.' + ext;
